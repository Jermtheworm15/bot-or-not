...
Deno.serve(async (req) => {
  try {
    ...
    const allKeywords = [...aiKeywords, ...realKeywords];
    ...
    existingImages.forEach(img => {
      const url = img.data?.url || img.url;
      if (url) fetchedUrls.add(url);
    });

    // Batch 1: Generate AI images
    ...
    for (let i = 0; i < aiBatchSize && totalToFetch - newImages.length > 0; i++) {
      try {
        ...
        if (result?.url && !fetchedUrls.has(result.url)) {
          ...
          newImages.push({ url: result.url, is_bot: true, source: 'ai_generated' });
        }
      } catch (error) {
    ...
    // Batch 2: Fetch real images from Unsplash-like sources
    ...
    for (let i = 0; i < realBatchSize && totalToFetch - newImages.length > 0; i++) {
      try {
        ...
        const keyword = realKeywords[Math.floor(Math.random() * realKeywords.length)];
        ...
        if (!fetchedUrls.has(url)) {
          ...
          newImages.push({ url, is_bot: false, source: 'unsplash' });
        }
      } catch (error) {
